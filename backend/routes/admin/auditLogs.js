...
const {
  ...
  deleteAuditLog,
  deleteAllAuditLogs,
} = require("../../controllers/admin/auditLog");
...
router.post("/create", verifyAppwriteToken, isAdmin, async (req, res) => {
  try {
    await createAuditLog(req.body, req);
...
// ✅ DELETE: Remove a single audit log by ID
router.delete("/:id", verifyAppwriteToken, isAdmin, deleteAuditLog);

// ✅ DELETE: Remove ALL audit logs (⚠️ be careful!)
router.delete("/", verifyAppwriteToken, isAdmin, deleteAllAuditLogs);
